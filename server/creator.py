...
mycursor.execute("CREATE TABLE IF NOT EXISTS dados (`id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY, `VELOCIDADE` varchar(25) NOT NULL, `ANGULO` varchar(25) NOT NULL, `imagem` longblob NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=latin1 AUTO_INCREMENT=1;")
mydb.close()
...
images = [arq for arq in os.listdir("./data/database_images")]
cmd_vel = [arq for arq in os.listdir("./data/database_cmdvel")]
joystick = [arq for arq in os.listdir("./data/database_joy")]
...
for i in range(len(images)):
    ...
    joystick = open("./data/database_joy/" + addr[0:-5] + ".txt", 'r')
    ...
    image = open('./data/database_images/' + addr,'rb').read()
    
    query = "INSERT INTO dados(id,VELOCIDADE,ANGULO,imagem) VALUES(%s,%s,%s,%s)"
    cursor=db.cursor()
